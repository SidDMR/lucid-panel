...
local registerFavorite = (function()
    ...
    return function(label, trigger, sourceRow, sourceControl, displayedLabel)
        local starred=false
...
local returnButtonWidth=math.min(260,math.max(64,52+#"Return to Previous Position"*6.6))
returnRow:SetAttribute("LucidFavoriteStarX",returnButtonWidth-27)
local returnButton=create("TextButton",{Size=UDim2.new(0,returnButtonWidth,0,26),Position=UDim2.new(0.5,-returnButtonWidth/2,0,0),BackgroundColor3=Color3.fromRGB(62,52,92),
...
local function actionButton(textValue, callback, color)
    ...
    local compactWidth=math.min(260,math.max(64,52+#textValue*6.6))
    row:SetAttribute("LucidFavoriteStarX",compactWidth-27)
    local button = create("TextButton", {
        Size = UDim2.new(0, compactWidth, 0, 28), Position=UDim2.new(0.5,-compactWidth/2,0,0),
        BackgroundColor3 = color or Color3.fromRGB(62, 52, 92),
...
-- Aim-style camera tracking, separate from the detachable Spectate Preview.
-- The camera remains attached to the local character and only rotates to keep
-- the selected player centered, similar to automatically steering Shift Lock.
do
    local followedPlayer=nil
    local followBinding="LucidCameraTarget_"..tostring(math.random(100000,999999))
    ...
    local function targetPosition(player)
        local character=player and player.Character
        if not character then return nil end
        local head=character:FindFirstChild("Head")
        local root=character:FindFirstChild("HumanoidRootPart")
        local part=(head and head:IsA("BasePart") and head) or (root and root:IsA("BasePart") and root)
    ...
    local function stopCameraFollow(restoreCamera)
        followedPlayer=nil
        RunService:UnbindFromRenderStep(followBinding)
    ...
    local function startCameraFollow(query)
        ...
        restoreLocalCamera()
        RunService:BindToRenderStep(followBinding,Enum.RenderPriority.Camera.Value+1,updateCameraTarget)
    ...
    track(workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
        if followedPlayer then restoreLocalCamera() end
    end))
...
local waypointSortMode="Alphabetical"
local lastSelectedWaypoint=nil
local pendingOverwrite=nil
local refreshWaypointDropdown = function() end
local function sortWaypointNames(names)
    local sortRoot=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    table.sort(names,function(a,b)
        if waypointSortMode=="Nearest" and sortRoot then
            local aDistance=(sortRoot.Position-waypoints[a].Position).Magnitude
            local bDistance=(sortRoot.Position-waypoints[b].Position).Magnitude
            if math.abs(aDistance-bDistance)>0.001 then return aDistance<bDistance end
        elseif waypointSortMode=="Numerical" then
            local aNumber=tonumber(tostring(a):match("[%+%-]?%d+%.?%d*"))
            local bNumber=tonumber(tostring(b):match("[%+%-]?%d+%.?%d*"))
            if aNumber and bNumber and aNumber~=bNumber then return aNumber<bNumber end
            if (aNumber~=nil)~=(bNumber~=nil) then return aNumber~=nil end
        end
...
actionButton("Sort Waypoints: Alphabetical",function(button)
    waypointSortMode=waypointSortMode=="Alphabetical" and "Numerical"
        or (waypointSortMode=="Numerical" and "Nearest" or "Alphabetical")
    button.Text="Sort Waypoints: "..waypointSortMode
    refreshWaypointDropdown()
end)
...
state.waypointApi={
    ...
    list=function()
        ...
        sortWaypointNames(names); return names
    end,
}
...
local mainStopRow=rowFrame(nextOrder(),30)
local stopEmoteButton=create("TextButton",{Size=UDim2.new(0,76,0,26),Position=UDim2.new(0.5,-38,0,0),
    BackgroundColor3=Color3.fromRGB(85,48,62),BorderSizePixel=0,Text="Stop Emote",
    TextColor3=Color3.fromRGB(235,230,245),TextSize=11,Font=Enum.Font.GothamSemibold,Parent=mainStopRow})
create("UICorner",{CornerRadius=UDim.new(0,6),Parent=stopEmoteButton})
stopEmoteButton.MouseButton1Click:Connect(function() if stopEmote then stopEmote() end end)
...
state.setEmotePlaybackSpeed=function(value,announce,previousValue)
    ...
    if announce and oldSpeed~=emoteSpeed then
        notifyLucid("Animation Speed",tostring(oldSpeed).." → "..tostring(emoteSpeed),Color3.fromRGB(75,210,120))
    end
end
emoteSpeedBox.FocusLost:Connect(function()
    state.setEmotePlaybackSpeed(emoteSpeedBox.Text,true)
end)
...
local emoteResultsHovered=false
emoteResults.MouseEnter:Connect(function() emoteResultsHovered=true end)
emoteResults.MouseLeave:Connect(function() emoteResultsHovered=false end)
local emoteScrollAction="LucidEmoteTwoRowScroll"
ContextActionService:BindActionAtPriority(emoteScrollAction,function(_,inputState,input)
    if not emoteResultsHovered or not emoteResults.Visible
        or (inputState~=Enum.UserInputState.Change and inputState~=Enum.UserInputState.Begin) then
        return Enum.ContextActionResult.Pass
    end
    local wheel=input.Position.Z
    if wheel==0 then return Enum.ContextActionResult.Sink end
    local maximum=math.max(0,emoteResults.AbsoluteCanvasSize.Y-emoteResults.AbsoluteWindowSize.Y)
    emoteResults.CanvasPosition=Vector2.new(emoteResults.CanvasPosition.X,
        math.clamp(emoteResults.CanvasPosition.Y-wheel*64,0,maximum))
    return Enum.ContextActionResult.Sink
end,false,3000,Enum.UserInputType.MouseWheel)
addCleanup(function() ContextActionService:UnbindAction(emoteScrollAction) end)
...
state.emoteModuleTabs.favoriteStopButton=create("TextButton",{Size=UDim2.new(0,76,0,26),Position=UDim2.new(0.5,-38,0,2),
    BackgroundColor3=Color3.fromRGB(85,48,62),BorderSizePixel=0,Text="Stop Emote",
...
state.initializePlayerEmoteBrowser=function()
    ...
    local playerStopButton=create("TextButton",{Size=UDim2.new(0,64,0,26),Position=UDim2.new(0.5,-32,0,0),
    ...
    local function speedControl(parent,withSlider,afterRow)
        ...
        if withSlider then
            ...
            slider.InputBegan:Connect(function(input)
                if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
                    dragInput=input; dragStartSpeed=state.emoteSpeed; update(input)
                end
            end)
            track(UserInputService.InputChanged:Connect(function(input)
            ...
            track(UserInputService.InputEnded:Connect(function(input)
                if input==dragInput then
                    dragInput=nil
                    state.setEmotePlaybackSpeed(state.emoteSpeed,true,dragStartSpeed)
                    dragStartSpeed=nil
                end
            end))
            box.FocusLost:Connect(function() state.setEmotePlaybackSpeed(box.Text,true) end)
            reset.MouseButton1Click:Connect(function() state.setEmotePlaybackSpeed(1,true) end)
            return
        end
        ...
        box.FocusLost:Connect(function()
            state.setEmotePlaybackSpeed(box.Text,true)
        end)
    end
...
if game.PlaceId==136070094363960 then
    ...
    local sekretPassLevelOneToThreeTargets={
        ...
        {-182.5,269,-145.118,9,364,1},{-177.5,269,-150.118,1,364,11},
    }
...
state.initializeCommandConsole=function()
    ...
    local function buildCommandCatalog()
        local catalog={
            ...
            {command="!fixcamera <player>",description="Keep your camera aimed at a player"},
            ...
            {command="!unfixcamera",description="Stop automatic camera aiming"},
    ...
    local function setConsoleBelow(below)
        ...
        if consoleBelow then
            ...
            console.Position=UDim2.new(0.5,0,1,8)
            browser.AnchorPoint=Vector2.new(0.5,0)
            browser.Position=UDim2.new(0.5,0,1,96)
            collapseButton.Text="▲"
        else
            console.AnchorPoint=Vector2.new(0.5,1)
            console.Position=UDim2.new(0.5,0,0,-8)
            browser.AnchorPoint=Vector2.new(0.5,1)
            browser.Position=UDim2.new(0.5,0,0,-88)
            collapseButton.Text="▼"
        end
    end
...
if not state.toolkitInitializationOk then
    ...
    useCategory("Home")
    create("TextLabel",{Size=UDim2.new(1,0,0,72),BackgroundColor3=Color3.fromRGB(72,30,42),
        BorderSizePixel=0,Text="Toolkit failed to load:\n"..tostring(state.toolkitInitializationError),
        TextWrapped=true,TextColor3=Color3.fromRGB(255,205,215),TextSize=10,Font=Enum.Font.Code,
        TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Top,
        LayoutOrder=nextOrder(),Parent=currentSection})
